def filter_by_currency(transactions, valute):
    '''Функция принимает список словарей и возвращает итератор,
    где валюта операции соответствует заданной'''
    filtered_transactions = filter(lambda t: t["operationAmount"]["currency"]["code"] == valute,
                               transactions)
...
def transaction_descriptions(transactions):
    '''Функция принимает список словарей и возвращает описание операций'''
    filtered_descriptions = filter(lambda t: t.get("description"),
                                   transactions)
    for t in filtered_descriptions:
        yield t["description"]
